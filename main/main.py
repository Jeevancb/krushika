...
def soiltest_results():
    ...
    recent_st = db.session.query(St_results).order_by(St_results.id.desc()).first()
...
def exp_dashboard():
    ...
    ae_labels = ['Seeds','Fertilizer','Irrigation','Labour','Transportation','Other']
    ae_values = [avglist(exp_data_seed),avglist(exp_data_fert),
    avglist(exp_data_irr),avglist(exp_data_lab),avglist(exp_data_transp),
    ...
    pdct_profit_labels = [(exp_year_uninc[-1]+1),(exp_year_uninc[-1]+2),(exp_year_uninc[-1]+3),(exp_year_uninc[-1]+4),(exp_year_uninc[-1]+5)]
    ...
    return render_template('exp_dashboard.html',r_labels=r_labels, r_values=r_values
    , cp_values=cp_values, ae_labels=ae_labels, ae_values=ae_values,pdct_profit_values=pdct_profit_values_r,pdct_profit_labels=pdct_profit_labels)
